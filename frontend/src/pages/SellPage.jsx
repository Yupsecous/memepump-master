...
import { FaDatabase, FaExclamationTriangle, FaPercentage, FaRegCopy } from "react-icons/fa";
...
export default function SellPage({ className }) {
    ...
    useEffect(() => {
        if (currentProject.teamWallets) {
            if (currentProject.teamWallets.length !== teamWalletChecked.length) {
                ...
                setTeamWalletSellPercent(currentProject.teamWallets.map(() => ""));
                setTeamWalletTransferOnSale(currentProject.teamWallets.map(() => false));
            }
    ...
    const handleOKSellPercent = (percent) => {
        ...
        for (let i = 0; i < newWalletSellPercent.length; i++) {
            if (walletChecked[i])
                newWalletSellPercent[i] = percent.toString();
        }
            ...
            <div className="flex flex-col">
                ...
                <div className="flex flex-row justify-between w-full gap-2 mt-3 mb-3 font-sans">
                    ...
                    <div className="flex flex-col justify-end gap-2 lg:items-center lg:flex-row">
                        ...
                        >
                            <FaPercentage className="text-lg text-red-normal" />
                            Set % Amount
                        </button>
                        <button
                            className="pl-3 pr-4 h-button rounded-[4px] justify-center items-center gap-1 inline-flex bg-[#262626] active:scale-95 transition duration-90 ease-in-out transform focus:outline-none text-xs font-medium text-center text-white uppercase disabled:text-gray-border disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
                                            ...
                                            currentProject.wallets.map((item, index) => {
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <td className="text-center">
                                                            <input
                                                                className="outline-none border border-gray-highlight font-medium text-gray-normal placeholder:text-gray-border text-xs px-2.5 bg-transparent text-center w-[150px] h-[26px] my-[2px]"
